...
@Component
public class DematAccountService implements IAccountService {

	private final ObjectMapper mapper;

	@Override
	public AccountDetails getAccountInfo(String name, String bankName, String countryCode)
			throws IOException {

		log.info("In Demat accounts");
		...
		for (JsonNode node : userDetails) {
			if (name.equalsIgnoreCase(node.at("/name").asText())
					&& "DEMAT".equalsIgnoreCase(node.at("/accountType").asText())) {
				log.info("Demat node: {}", node);
